...
class Amministratore extends Utente{
     ...
     * del database è già presente un amministratore con la stessa email
     * inserita in fase di registrazione
     */
    async verificaAmministratoreEsistente(){
        letAmministratoreDB = await modelloAmministratore.find({ email: this.email }).exec();
        if(AmministratoreDB.length){ //if null
            return false;
        }else{
            return true;
        }
